...
@pytest.mark.asyncio
async def test_create_work(test_client: AsyncClient):
    work_data = {
        "name": "Tech Corp",
    ...
    assert data["summary"] == "Developed amazing software"
    assert "Increased efficiency by 50%" in data["highlights"]


@pytest.mark.asyncio
async def test_get_work(test_client: AsyncClient):
    work_data = {
        "name": "Tech Corp",
        "position": "Software Engineer",
    }
...
@pytest.mark.asyncio
async def test_get_all_work(test_client: AsyncClient):
    ...
    data = response.json()
    assert len(data) == 2
    assert any(work["name"] == "Tech Corp" for work in data)
    assert any(work["name"] == "Startup Inc" for work in data)


@pytest.mark.asyncio
async def test_update_work(test_client: AsyncClient):
    work_data = {
        "name": "Tech Corp",
        "position": "Software Engineer",
    }
    ...
    assert data["summary"] == "Promoted to senior role"


@pytest.mark.asyncio
async def test_delete_work(test_client: AsyncClient):
    work_data = {
        "name": "Tech Corp",
        "position": "Software Engineer",
    }
    ...
    get_response = await test_client.get(f"/work/{work_id}")
    assert get_response.status_code == 404


@pytest.mark.asyncio
async def test_get_work_not_found(test_client: AsyncClient):
    response = await test_client.get("/work/nonexistent-id")
    assert response.status_code == 404


@pytest.mark.asyncio
async def test_update_work_not_found(test_client: AsyncClient):
...
@pytest.mark.asyncio
async def test_delete_work_not_found(test_client: AsyncClient):

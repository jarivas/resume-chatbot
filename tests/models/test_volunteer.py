...
@pytest.mark.asyncio
async def test_create_volunteer(test_client: AsyncClient):
    volunteer_data = {
    ...
    assert "Organized events" in data["highlights"]


@pytest.mark.asyncio
async def test_get_volunteer(test_client: AsyncClient):
    volunteer_data = {
        "organization": "Red Cross",
        "position": "Volunteer",
    }
...
@pytest.mark.asyncio
async def test_get_all_volunteer(test_client: AsyncClient):
    ...
    data = response.json()
    assert len(data) == 2
    assert any(vol["organization"] == "Red Cross" for vol in data)
    assert any(vol["organization"] == "Green Peace" for vol in data)


@pytest.mark.asyncio
async def test_update_volunteer(test_client: AsyncClient):
    volunteer_data = {
        "organization": "Red Cross",
        "position": "Volunteer",
    }
    ...
    assert data["summary"] == "Led volunteer team"


@pytest.mark.asyncio
async def test_delete_volunteer(test_client: AsyncClient):
    volunteer_data = {
        "organization": "Red Cross",
        "position": "Volunteer",
    }
    ...
    get_response = await test_client.get(f"/volunteer/{volunteer_id}")
    assert get_response.status_code == 404


@pytest.mark.asyncio
async def test_get_volunteer_not_found(test_client: AsyncClient):
    response = await test_client.get("/volunteer/nonexistent-id")
    assert response.status_code == 404


@pytest.mark.asyncio
async def testver_update_volunteer_not_found(test_client: AsyncClient):
...
@pytest.mark.asyncio
async def test_delete_volunteer_not_found(test_client: AsyncClient):

...
@pytest.mark.asyncio
async def test_references_post_endpoint(test_client: AsyncClient):
    references_data = {
        "name": "John Doe",
        "reference": "Excellent developer with strong problem-solving skills",
    }

    response = await test_client.post("/references", json=references_data)
...
@pytest.mark.asyncio
async def test_references_get_by_id_endpoint(test_client: AsyncClient):
    references_data = {
        "name": "Jane Smith",
        "reference": "Great team player",
    }

    create_response = await test_client.post("/references", json=references_data)
...
@pytest.mark.asyncio
async def test_references_get_all_endpoint(test_client: AsyncClient):
    references_data1 = {"name": "Ref 1", "reference": "Text 1"}
    references_data2 = {"name": "Ref 2", "reference": "Text 2"}

    await test_client.post("/references", json=references_data1)
    await test_client.post("/references", json=references_data2)
...
@pytest.mark.asyncio
async def test_references_patch_endpoint(test_client: AsyncClient):
    references_data = {
...
@pytest.mark.asyncio
async def test_references_delete_endpoint(test_client: AsyncClient):
    references_data = {
...
@pytest.mark.asyncio
async def test_references_get_not_found_endpoint(test_client: AsyncClient):
    response = await test_client.get("/references/nonexistent-id")
    assert response.status_code == 404


@pytest.mark.asyncio
async def test_references_patch_not_found_endpoint(test_client: AsyncClient):
...
@pytest.mark.asyncio
async def test_references_delete_not_found_endpoint(test_client: AsyncClient):
